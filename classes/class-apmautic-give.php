...
if ( ! class_exists( 'APMautic_Give' ) ) :
	...
	class APMautic_Give {
		...
		public function set_constants() {
			...
			define( 'AUTOMATEPLUS_MAUTIC_GIVE_FILE', trailingslashit( dirname( dirname( __FILE__ ) ) ) . 'automate-mautic-give.php' );
			define( 'AUTOMATEPLUS_MAUTIC_GIVE_DIR', plugin_dir_path( AUTOMATEPLUS_MAUTIC_GIVE_FILE ) );
			define( 'AUTOMATEPLUS_MAUTIC_GIVE_URL', plugins_url( '/', AUTOMATEPLUS_MAUTIC_GIVE_FILE ) );
		}
		...
		public function apm_give_notices() {
			...
			if ( ! class_exists( 'APMautic_loader' ) ) {
				...
				$url = network_admin_url() . 'plugin-install.php?s=AutomatePlug+-+Mautic+for+WordPress&tab=search';
				// translators: %s: plugin install link.
				printf( __( '<div class="update-nag bsf-update-nag">Please install <i><a href="%s">AutomatePlug - Mautic for WordPress</a></i> plugin in order to use Automate Mautic Give.</div>', 'automate-mautic-give' ), $url );
			}
			...
			if ( ! class_exists( 'Give' ) ) {
				...
				// translators: %s: plugin install link.
				printf( __( '<div class="update-nag bsf-update-nag">Please install <i><a href="%s">Give - WordPress Donation Plugin</a></i> plugin in order to use Automate Mautic Give.</div>', 'automate-mautic-give' ), $url );
			}
		}
		...
		public function select_all_forms( $select = null ) {
			...
			$options = array(
				'' => __( 'Select Form', 'automate-mautic-give' ),
			);
		...
		public function send_customers( $footer_text ) {
			...
			if ( 'settings_page_automate-mautic' == $screen->id ) {
				...
				$refresh_text .= __( 'Send Give donors to Mautic', 'automate-mautic-give' );
				$refresh_text .= '</a>';
		...
		public function give_mautic_config( $payment ) {
			...
			$customer_failed = array(
				'add_segment' => array(),
				'remove_segment' => array(),
			);
			$customer_revoked = array(
				'add_segment' => array(),
				'remove_segment' => array(),
			);
			$customer_abandoned = array(
				'add_segment' => array(),
				'remove_segment' => array(),
			);
		...
		public static function update_give_tab_content() {
			...
			if ( isset( $_POST['amp-mautic-nonce-give'] ) && wp_verify_nonce( $_POST['amp-mautic-nonce-give'], 'ampmauticgive' ) ) {
				...
				$give_options = APMautic_Helper::get_amp_options();
				$give_options['amp_give_gateway'] = false;
				$give_options['amp_give_payment'] = false;
				$give_options['amp_give_form_tag'] = false;
				$give_options['amp_give_proactive_abandoned'] = false;
				$give_options['apm_give_remove_segment'] = false;
				$give_options['remove_segment_ap'] = false;
		...
		public function render_give_tab_content( $active ) {
			...
			if ( 'give_mautic' == $active ) {
				...
				<table class="form-table widefat">
					<thead>
					<tr>
						<td class="row-title"><b><?php esc_attr_e( 'Condition', 'automate-mautic-give' ); ?></b></td>
						<td><b><?php esc_attr_e( 'Add to Segment', 'automate-mautic-give' ); ?></b></td>
					</tr>
					</thead>
					<tbody>
					<tr>
						<td class="row"><?php _e( 'After complete donation, add users to:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action ); ?><input type="checkbox" style="margin-left: 2%;" class="amp-enabled-panels" name="remove_segment_ap" value="" <?php checked( 1, $remove_segment_ap ); ?> ><?php _e( 'Remove users from all segments', 'automate-mautic-give' ); ?></td>
					</tr>
					<tr>
						<td class="row"><label for="tablecell"><?php _e( 'Add customer with failed order to:', 'automate-mautic-give' ); ?></label></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_failed ); ?></td>
					</tr>
					<tr>
						<td class="row"><?php _e( 'Add customer with revoked order to:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_revoked ); ?></td>
					</tr>
					<tr>
						<td class="row"><?php _e( 'Add customer with abandoned order to:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_hold ); ?><input type="checkbox" style="margin-left: 2%;" class="amp-enabled-panels" name="amp_give_proactive_abandoned" value="" <?php checked( 1, $proactive_tracking ); ?> ><?php _e( 'Enable Proactive Abandonment Tracking', 'automate-mautic-give' ); ?></td>
					</tr>
					<tr>
						<td class="row"><?php _e( 'Add customer with pending order to:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_pending ); ?></td>
					</tr>
					<tr>
						<td class="row"><?php _e( 'Add refunded users in:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_refund ); ?><input type="checkbox" style="margin-left: 2%;" class="amp-enabled-panels" name="apm_give_remove_segment" value="" <?php checked( 1, $apm_give_remove_segment ); ?> ><?php _e( 'Remove users from all segments', 'automate-mautic-give' ); ?></td>
					</tr>
					<tr>
						<td class="row"><?php _e( 'Add cancelled donors in:', 'automate-mautic-give' ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $ss_seg_action_cancel ); ?></td>
					</tr>
					</tbody>
				</table>
				...
				<table class="form-table widefat">
					<thead>
						<tr>
							<td class="row-title" style="width: 40%;"><b><?php esc_attr_e( 'Condition', 'automate-mautic-give' ); ?></b></td>
							<td class="row-title" style="width: 30%;"><b><?php esc_attr_e( 'Give Form', 'automate-mautic-give' ); ?></b></td>
							<td style="width: 30%;"><b><?php esc_attr_e( 'Add to Segment', 'automate-mautic-give' ); ?></b></td>
						</tr>
					</thead>
					<tbody>
					<tr>
						<td class="row"><?php _e( 'Add users who donate with specific form', 'automate-mautic-give' ); ?></td>
						<td class="row"><?php self::select_all_forms( $give_form ); ?></td>
						<td><?php APMautic_Services::select_all_segments( $seg_action_form ); ?></td>
					</tr>
					</tbody>
				</table>
				...
				<table class="form-table widefat">
					<thead>
						<tr>
							<td class="row-title"><b><?php esc_attr_e( 'Add Mautic Tags', 'automate-mautic-give' ); ?></b></td>
						</tr>
					</thead>
					<tbody>
					<tr>
						<td class="row"><input type="checkbox" class="amp-enabled-panels" name="amp_give_form_tag" value="" <?php checked( 1, $give_form_tag ); ?> ><?php _e( 'Automatically add Give form title as a tag in Mautic', 'automate-mautic-give' ); ?></td>
					</tr>
					<tr>
						<td class="row"><input type="checkbox" class="amp-enabled-panels" name="amp_give_gateway" value="" <?php checked( 1, $give_gateway ); ?> ><?php _e( 'Automatically add Give payment gateway as a tag in Mautic', 'automate-mautic-give' ); ?></td>
					</tr>
					<tr>
...
', 'automate-mautic-give' ); ?></td>
					</tr>
					</tbody>
				</table>

				<p class="submit">
					<input type="submit" name="save-give-tab" id="save-amp-settings" class="button-primary button button-large" value="<?php esc_attr_e( 'Save Settings', 'automate-mautic-give' ); ?>" />
